...
class Submission:
    ...
    def get_review_messages(self):
        try:
            submission_review_messages = execute_database_command('''SELECT srm.submission_id, srm.teacher_id, srm.message_id, srm.created_utc, srm.id FROM
            submissions s JOIN submission_review_messages srm ON s.id = srm.submission_id WHERE s.id=%s''', (self.id, ))[0]
            return [SubmissionReviewMessage(srm[0], srm[1], srm[2], srm[3], srm[4]) for srm in submission_review_messages]
        except IndexError:
            return None

    review_messages = property(get_review_messages)
